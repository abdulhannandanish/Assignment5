...
for i in range (0,n-1):
	for j in range (0,n-1-i):
            if Priority_No[j]>Priority_No[j+1]:
                Priority_No[j], Priority_No[j + 1] = Priority_No[j + 1], Priority_No[j]
                Arrival_Time[j], Arrival_Time[j + 1] = Arrival_Time[j + 1], Arrival_Time[j]
                Burst_Time[j], Burst_Time[j + 1] = Burst_Time[j + 1], Burst_Time[j]

print("Priority No ", "Arrival Time,Burst Time","Turn Around Item","Waiting Time")
for i in range(0,n):
	Turn_Around[i] =Arrival_Time[i]+Burst_Time[i]
	Waiting_Time[i]=Turn_Around[i]-Arrival_Time[i]
	print(Priority_No[i],Arrival_Time[i],Burst_Time[i],Turn_Around[i],Waiting_Time[i])
